...
func main() {
	/*
		...
		语法结构：
			1.创建map
				...
				var map3 = map[key类型]value类型{key:value,key:value,key:value,key:value}
	...
	*/
	...
	var map3 = map[string]int{"Go": 99, "Python": 87, "Java": 79, "Html": 93} //初始化并赋值
	fmt.Println(map1)
	fmt.Println(map2)
	fmt.Println(map3)
	...
	//3.存储键值对到map中
	//map1[key] = value
	map1[1] = "hello" //panic:assignmt to entry in nil map
	map1[2] = "world"
	map1[3] = "memeda"
	map1[4] = "王二狗"
	map1[5] = "ruby"
	...
	//根据key获取对应的value，如果key存在，获取数值，如果key不存在，获取的是value的类型的零值
